...
// --- Firebase bootstrap with safe fallbacks ---
const parsedConfig = (() => {
  const raw = (typeof __firebase_config !== 'undefined' && __firebase_config) || import.meta.env.VITE_FIREBASE_CONFIG;
  if (!raw) return null;
  try {
...
const appId = typeof __app_id !== 'undefined' && __app_id ? __app_id : 'aditya-university-slots';
...
export default function App() {
  ...
  const updateSlot = async (day, slot, busy, work) => {
    if (!user || !currentUserProfile || !db) return;
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 text-center px-6">
        <div className="bg-white shadow-lg rounded-2xl p-8 border">
          <h1 className="text-2xl font-bold mb-3">Firebase config missing</h1>
          <p className="text-slate-600 mb-4 text-sm">
            Provide a Firebase config via the global variable __firebase_config or env var VITE_FIREBASE_CONFIG
            (stringified JSON). Reload after setting it.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {view === 'directory' && (
          <DirectoryView facultyData={facultyData} searchQuery={searchQuery} setSearchQuery={setSearchQuery} onDownload={downloadCSV} />
        )}

        {view === 'register' && !currentUserProfile && <RegisterView onRegister={handleRegister} />}

        {view === 'schedule' && currentUserProfile && <ScheduleView profile={currentUserProfile} onUpdate={updateSlot} />}
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8">
        ...
        >
          ...
          <div className="flex gap-2">
            <button type="button" onClick={onClose} className="flex-1 py-3 font-bold text-slate-500 hover:bg-slate-100 rounded-xl transition-colors">
              Cancel
            </button>
            <button type="submit" className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-100">
              Login
            </button>
          </div>
        </form>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-bold transition-all ${
                showFreeOnly ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
              }`}
            >
                ...
                <table className="w-full text-left text-sm">
                  <thead>
                    <tr className="bg-slate-100/50">
                      ...
                      {SLOTS.map((s) => (
                        <th key={s} className="px-4 py-2 font-semibold border-b text-center text-[10px] leading-tight min-w-[80px]">
                          {s}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {DAYS.map((day) => (
                      <tr
                        key={day}
                        className={`hover:bg-slate-50 transition-colors ${showFreeOnly && day === filterDay ? 'bg-blue-50/30' : ''}`}
                      >
                        <td
                          className={`px-4 py-3 font-semibold border-r border-b ${
                            showFreeOnly && day === filterDay ? 'text-blue-700' : 'bg-slate-50/30'
                          }`}
                        >
                          {day}
                        </td>
                        {SLOTS.map((slot) => {
                          ...
                          return (
                            <td key={slot} className={`px-1 py-1 border-b ${isHighlighted ? 'bg-blue-50' : ''}`}>
                              <div
                                className={`p-2 rounded-lg text-[10px] font-bold text-center h-full min-h-[40px] flex items-center justify-center transition-all ${
                                  status.busy
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50">
                ...
                {SLOTS.map((s) => (
                  <th key={s} className="px-6 py-4 font-bold border-b text-center text-xs uppercase tracking-wider">
                    {s}
                  </th>
                ))}
              </tr>
                ...
                <tr key={day} className="group">
                  ...
                  {SLOTS.map((slot) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`p-3 rounded-2xl min-h-[60px] flex flex-col items-center justify-center text-center transition-all ${
                            status.busy
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-sm overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex gap-3">
                <button onClick={() => setEditingSlot(null)} className="flex-1 py-3 text-slate-600 font-bold hover:bg-slate-100 rounded-xl">
                  Cancel
                </button>
                <button
                  onClick={saveChange}
                  className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-100"
                >
                  Save Status
                </button>
              </div>
        ...
        <div className="md:col-span-1 space-y-6">
          <div className="bg-white p-6 rounded-3xl border shadow-sm">
            ...
            <div className="space-y-3">
              ...
              <label
                className={`w-full flex items-center justify-center gap-2 py-3 px-4 bg-blue-600 text-white rounded-xl text-sm font-bold cursor-pointer hover:bg-blue-700 transition-all ${
